...
async def client(mock_fetcher, mock_crypto):
    app.dependency_overrides[SignatureAssetFetcher] = lambda: mock_fetcher
...
async def test_crypto_value_error_returns_422(client, mock_fetcher, mock_crypto, valid_payload):
    ...
    mock_crypto.apply_signature.side_effect = ValueError("Invalid PKCS#12 container or wrong password")
    
    response = await client.post("/api/v1/sign", json=valid_payload)
    
    assert response.status_code == 422
    assert response.json()["detail"] == "Invalid PKCS#12 container or wrong password"